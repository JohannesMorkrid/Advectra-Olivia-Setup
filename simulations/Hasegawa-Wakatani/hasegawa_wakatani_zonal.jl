...
function NonLinear!(du, u, operators, parameters, t)
    ...
    coupling_term = C * (ϕ - n)
    remove_zonal_modes!(coupling_term, domain) # This is what makes it the modified HW
    dn .= poisson_bracket(n, ϕ) - diff_y(ϕ) + coupling_term
    dΩ .= poisson_bracket(Ω, ϕ) + coupling_term
end
...
for (C, γ) in zip(Cs, gammas)
    ...
    # Output
    output = Output(prob; filename="/cluster/work/projects/nn12110k/joemork/Hasegawa-Wakatani/HW_C-$(C)_hyper_zonal.h5",
